...
std::deque <int> PmergeMe::fordJohnsonDeque(const std::deque <int> &input)
{
	...
	createDequePairs(input, pairs, winners, losers);

	std::deque<int> sortedWinners = fordJohnsonDeque(winners);

	std::deque<int> alignedLosers = alignDequeLosers(sortedWinners, pairs);

	return insertDequeElements(sortedWinners, alignedLosers, straggler, hasStraggler);
}
...
void PmergeMe::createDequePairs(const std::deque<int> &input,
	std::deque<std::pair<int, int> > &pairs,
	std::deque<int> &winners,
	std::deque<int> &losers)
{
...
std::deque<int> PmergeMe::alignDequeLosers(const std::deque<int> &sortedWinners, const std::deque<std::pair<int, int> > &pairs)
{
	std::deque<int> aligned;
	for (size_t i = 0; i < sortedWinners.size(); i++)
	{
		for (size_t j = 0; j < pairs.size(); j++)
		{
...
std::deque<int> PmergeMe::insertDequeElements(const std::deque<int> &sortedWinners,
